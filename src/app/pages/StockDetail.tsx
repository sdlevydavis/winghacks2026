...
export function StockDetail() {
  ...
  const holding = stock ? userData.portfolio[stock.symbol] : undefined;
      ...
      switch (achievement.id) {
        ...
        case 'diversified':
          const sectors = new Set(
            Object.keys(updatedData.portfolio).map(sym => 
              stocks.find(s => s.symbol === sym)?.sector
            )
          );
  ...
  const handleBuy = () => {
    ...
    if (updatedPortfolio[stock.symbol]) {
      ...
      const totalCost = (existing.shares * existing.averagePrice) + (sharesNum * stock.currentPrice);
      updatedPortfolio[stock.symbol] = {
        shares: totalShares,
          ...
          <Card className="p-4 bg-blue-50 border-blue-200">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div>
                ...
                <p className="font-bold text-lg">${(holding.shares * stock.currentPrice).toFixed(2)}</p>
              </div>
              <div>
                <p className="text-gray-600">Profit/Loss</p>
                <p className={`font-bold text-lg ${
                  (stock.currentPrice - holding.averagePrice) >= 0 ? 'text-green-600' : 'text-red-600'
                }`}>
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                You own {holding?.shares || 0} shares
              </p>
            </div>
            <div className="bg-gray-50 p-3 rounded-lg space-y-2 text-sm">
              <div className="flex justify-between">
              ...
              {holding && (
                <div className="flex justify-between text-xs">
                  ...
                  <span className={
                    (stock.currentPrice - holding.averagePrice) * sharesNum >= 0 
                      ? 'text-green-600' 
                      : 'text-red-600'
                  }>
